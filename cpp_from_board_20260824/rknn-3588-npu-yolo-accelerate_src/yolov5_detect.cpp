...
int main(int argc, char **argv)
{
    if (argc < 3)
    {
        NN_LOG_ERROR("usage: %s <model.rknn> <video> [threads]", argv[0]);
        return 1;
    }
    ...
    if (num_threads <= 0 || g_pool->setUp(model_file, num_threads) != NN_SUCCESS)
    {
        NN_LOG_ERROR("failed to initialize YOLO thread pool");
        delete g_pool;
        g_pool = nullptr;
        return 1;
    }
